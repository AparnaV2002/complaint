...
app.post('/edit/:id',async(req,res)=>{
    const {id}=req.params;
    const {name,photourl,mobile,email,productname,description,solved} = req.body
    console.log({...req.params})
    try {
        await User.findOneAndUpdate({_id:id},{name,photourl,mobile,email,productname,description,solved})
        res.status(200).send('success')
    } catch (error) {
        res.status(500).send(error.message)
    }
